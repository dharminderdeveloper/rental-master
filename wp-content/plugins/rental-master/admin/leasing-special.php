...
class RM_Leasing_Special{
	...
	public function __construct(){		
		...
		add_action('cmb2_admin_init',array($this,'rm_leasing_special_register_metabox'));		
		add_action( 'save_post',array( $this,'rm_leasing_special_update_date' ),10,3);
	}
	...
	public function get_expired_leasing_special(){
		...
		$expired_leasing	=	$wpdb->get_results( "SELECT * from $table_name WHERE `meta_key`='rm_leasing_new_expire_date' and `meta_value`<= '".date( 'Y-m-d' )."'");
		...
		if( count( $expired_leasing ) > 0 ){
			foreach( $expired_leasing as $special ){				
				...
				RM_Previous_Leasing_Special::save_previous_leasing_special( $prSpecial,'expired');
	...
	public function update_leasing_special_date_column(){			
		...
		if( $is_leasing_update != 1){			
			...
			foreach( $cc_query->posts as $lSepcial){				
				$expire_date 	=	get_post_meta( $lSepcial->ID,'rm_leasing_expire_date',true );
				$date_array 	=	explode(  '/', $expire_date );				
				$new_expire_date =  $date_array[2].'-'.$date_array[0].'-'.$date_array[1];	
				update_post_meta( $lSepcial->ID, 'rm_leasing_new_expire_date',$new_expire_date  );		
			}
	...
	function rm_leasing_special_update_date(  $post_id, $post, $update ) {		
		...
		if ($post->post_type == 'leasing' &&  $pr_leasing_counter!=1 && $update == 1 && $post->post_status == 'publish' && isset( $_POST[ 'save' ])  ) {
			...
			//	Get Expriy Date
			$expire_date 	=	sanitize_text_field($_POST['rm_leasing_expire_date']);
			$date_array 	=	explode(  '/', $expire_date );				
			$new_expire_date =  $date_array[2].'-'.$date_array[0].'-'.$date_array[1];	
			update_post_meta( $post_id, 'rm_leasing_new_expire_date',$new_expire_date  );
			
			//	Fetch Leasing Special Detail
			$prSpecial		=	$this->get_leasing_special_detail( $post_id );	
			
			//	Save Previous Leasing Special
			RM_Previous_Leasing_Special::save_previous_leasing_special( $prSpecial,'Updated');			 				
	...
	public function get_leasing_special_detail( $leasingID = '' ){
		$publish_date 	=	get_post_meta($leasingID,'rm_leasing_publish_date', true);
		$expire_date 	=	get_post_meta($leasingID,'rm_leasing_expire_date', true);
		$desc 			=	get_post_meta($leasingID,'rm_leasing_description', true);
		$aids 			=	get_post_meta($leasingID,'rm_leasing_apartment', true);
	...
	function rm_leasing_special_register_metabox(){
...
add_action('plugins_loaded',array('RM_Leasing_Special','init'));
